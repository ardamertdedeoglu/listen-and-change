...
class SpeechService {
  ...
  async transcribeAudio(audioFilePath) {
    try {
      console.log(
        `Starting real speech-to-text transcription for: ${audioFilePath}`
      );
      ...
      if (!fs.existsSync(this.scriptPath)) {
        console.log(
          "Python speech recognition script not found, using mock data..."
        );
      ...
      try {
        ...
        if (result.words && result.words.length > 0) {
          ...
          const invalidWords = result.words.filter(
            (word) => word.startTime === 0 && word.endTime === 0
          );

          if (invalidWords.length > 0) {
            console.log(
              `⚠️ Found ${invalidWords.length} words with invalid timestamps (0.0-0.0)`
            );

            // Try to fix invalid timestamps
            result.words = this.fixInvalidTimestamps(
              result.words,
              result.duration
            );
          }
        ...
        console.log(
          "⚠️ Real transcription failed, falling back to mock data:",
          error.message
        );
        return this.getMockTranscription(audioFilePath);
      }
  ...
  fixInvalidTimestamps(words, duration) {
    ...
    for (let i = 0; i < fixedWords.length; i++) {
      ...
      if (word.startTime === 0 && word.endTime === 0) {
        ...
        while (
          nextValidIndex < fixedWords.length &&
        ...
        const nextValidStartTime =
          nextValidIndex < fixedWords.length
            ? fixedWords[nextValidIndex].startTime
            : duration;
      ...
      pythonProcess.on("close", (code) => {
        if (code === 0) {
          try {
            ...
            reject(
              new Error(
                `Failed to parse transcription result: ${parseError.message}`
              )
            );
          }
        } else {
