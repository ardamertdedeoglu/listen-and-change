...
class AudioService {
    ...
    async processAudioReplacements(audioFilePath, replacements) {
        try {
            ...
            const outputFilename = `processed-${Date.now()}-${Math.round(Math.random() * 1E9)}.wav`;
            ...
            try {
                ...
                await this.performWordReplacements(audioFilePath, outputPath, replacements);
                ...
                console.log('⚠️ Real audio processing failed, using basic processing:', error.message);
    ...
    async performWordReplacements(inputPath, outputPath, replacements) {
        ...
        try {
            ...
            const sortedReplacements = replacements
                .filter(r => r.startTime !== undefined && r.endTime !== undefined && r.startTime < r.endTime)
            ...
            const validatedReplacements = this.validateReplacements(sortedReplacements);
            console.log(`After validation: ${validatedReplacements.length} valid replacements`);
            ...
            for (let i = 0; i < validatedReplacements.length; i++) {
                ...
                console.log(`Processing replacement ${i + 1}/${validatedReplacements.length}: "${replacement.originalWord}" → "${replacement.replacementText}" at ${replacement.startTime}s-${replacement.endTime}s`);
                ...
                try {
                    ...
                    console.log(`Replacing audio segment from ${replacement.startTime}s to ${replacement.endTime}s`);
                    await this.replaceAudioSegment(currentFile, ttsPath, replacement.startTime, replacement.endTime, stepOutput);
                    ...
                    console.error(`❌ Failed to process replacement ${i + 1}:`, error.message);
    ...
    validateReplacements(replacements) {
        ...
        for (const replacement of sorted) {
            ...
            if (replacement.startTime < lastEndTime) {
                console.log(`⚠️ Skipping overlapping replacement: "${replacement.originalWord}" at ${replacement.startTime}s-${replacement.endTime}s (overlaps with previous replacement ending at ${lastEndTime}s)`);
                continue;
            }
            
            // Skip if start time equals end time
            if (replacement.startTime === replacement.endTime) {
                console.log(`⚠️ Skipping replacement with zero duration: "${replacement.originalWord}" at ${replacement.startTime}s-${replacement.endTime}s`);
                continue;
            }
            
            // Skip if duration is too short (less than 0.1 seconds)
            if (replacement.endTime - replacement.startTime < 0.1) {
                console.log(`⚠️ Skipping replacement with too short duration: "${replacement.originalWord}" at ${replacement.startTime}s-${replacement.endTime}s`);
                continue;
            }
    ...
    async replaceAudioSegment(inputPath, replacementPath, startTime, endTime, outputPath) {
        return new Promise((resolve, reject) => {
            ...
            const tempReplacementPath = replacementPath.replace('.wav', '_normalized.wav');
            ...
            ffmpeg(replacementPath)
                ...
                .on('end', () => {
                    ...
                    this.performAudioSplicing(inputPath, tempReplacementPath, startTime, endTime, outputPath)
                        .then(() => {
                            // Clean up temp file
    ...
    async performAudioSplicing(inputPath, replacementPath, startTime, endTime, outputPath) {
        return new Promise((resolve, reject) => {
