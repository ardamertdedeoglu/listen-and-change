...
class NLPService {
  ...
  getAllInappropriateWords() {
    ...
    for (const category in this.inappropriateWords) {
      if (
        this.inappropriateWords.hasOwnProperty(category) &&
        Array.isArray(this.inappropriateWords[category])
      ) {
  ...
  findTargetWords(text, targetWords) {
    ...
    words.forEach((word, index) => {
      ...
      if (index < words.length - 1) {
        ...
        const twoWordPhrase = `${cleanWord} ${words[index + 1].replace(
          /[^\w]/g,
          ""
        )}`;
        ...
        if (index < words.length - 2) {
          const threeWordPhrase = `${cleanWord} ${words[index + 1].replace(
            /[^\w]/g,
            ""
          )} ${words[index + 2].replace(/[^\w]/g, "")}`;
  ...
  generateReplacements(wordsFound) {
    return wordsFound.map((item) => {
      ...
      const suggestions = this.replacementSuggestions[word] || [
        "[BEEP]",
        "*****",
      ];
  ...
  generateRecommendations(wordsFound) {
    ...
    if (wordsFound.length === 0) {
      recommendations.push(
        "No inappropriate content detected. Audio is family-friendly."
      );
    } else {
      recommendations.push(
        `Found ${wordsFound.length} word(s) that may need replacement.`
      );
      recommendations.push(
        "Consider replacing these words with family-friendly alternatives."
      );

      if (wordsFound.length > 5) {
        recommendations.push(
          "High number of replacements needed. Consider reviewing the entire content."
        );
      }
    }
